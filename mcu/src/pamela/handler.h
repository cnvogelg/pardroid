...
typedef u08  (*hnd_attach_func_t)(u08 chn);
typedef void (*hnd_work_func_t)(u08 chn);
typedef u08  (*hnd_detach_func_t)(u08 chn);

typedef u08  (*hnd_open_func_t)(u08 chn);
typedef void (*hnd_close_func_t)(u08 chn);
typedef void (*hnd_reset_func_t)(u08 chn);
typedef void (*hnd_rx_request_func_t)(u08 chn);

typedef void (*hnd_read_begin_func_t)(u08 chan, u16 size, u16 frag_offset, u16 total);
typedef void (*hnd_read_end_func_t)(u08 chan);

typedef u08 *(*hnd_write_begin_func_t)(u08 chan, u16 size, u16 frag_offset, u16 total);
typedef void (*hnd_write_end_func_t)(u08 chan, u16 size);

typedef u16  (*hnd_check_mtu_func_t)(u08 new_mtu);
...
struct handler {
  ...
  hnd_check_mtu_func_t          check_mtu;
};
...
#define HANDLER_GET_PROPERTIES(hnd)  read_rom_word(&hnd->properties)

#define HANDLER_FUNC_ATTACH(hnd)      ((hnd_attach_func_t)read_rom_rom_ptr(&hnd->attach))
#define HANDLER_FUNC_WORK(hnd)        ((hnd_work_func_t)read_rom_rom_ptr(&hnd->work))
#define HANDLER_FUNC_DETACH(hnd)      ((hnd_detach_func_t)read_rom_rom_ptr(&hnd->detach))
...
#define HANDLER_FUNC_RX_REQUEST(hnd)  ((hnd_rx_request_func_t)read_rom_rom_ptr(&hnd->rx_request))
