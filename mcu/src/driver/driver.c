...
{
  rom_pchar ptr = read_rom_rom_ptr(&driver_table[num]);
  return (driver_ptr_t)ptr;
}
...
void driver_init(u08 num)
{
  for(u08 did=0;did<num;did++) {
    ...
    rom_pchar ptr = read_rom_rom_ptr(&drv->init_func);
    drv_init_func_t f = (drv_init_func_t)ptr;
    if(f != 0) {
      if(f(did)) {
...
void driver_work(u08 num)
{
  for(u08 did=0;did<num;did++) {
    driver_ptr_t drv = get_driver(did);
    driver_data_t *data = DRIVER_GET_DATA(did);
    rom_pchar ptr = read_rom_rom_ptr(&drv->work_func);
    drv_work_func_t f = (drv_work_func_t)ptr;
    if(f != 0) {
      f(did, data->flags);
    }
  }
...
u08 driver_open(u08 did)
{
  ...
  if(did < max) {
    ...
    drv_open_func_t f = (drv_open_func_t)ptr;
    if(f != 0) {
      return f(did);
    } else {
      return DRIVER_OK;
...
void driver_close(u08 did)
{
  ...
  if(did < max) {
    ...
    drv_close_func_t f = (drv_close_func_t)ptr;
    if(f != 0) {
      return f(did);
    }
  }
...
u16 driver_read(u08 did, u08 *buf, u16 size)
{
  ...
  if(did < max) {
    ...
    drv_read_func_t f = (drv_read_func_t)ptr;
...
u16 driver_write(u08 did, u08 *buf, u16 size)
{
  ...
  if(did < max) {
    ...
    drv_write_func_t f = (drv_write_func_t)ptr;
    return f(did, buf, size);
  } else {
    return 0;
  }
